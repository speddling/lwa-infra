...
- **Secrets** — Ansible Vault · single consolidated vault at `ansible/vars/vault.yml`
- **Variables** — Single source of truth at `ansible/vars/main.yml` — all IPs, ports, paths, and service config in one place
...
GitHub Actions pipelines on self-hosted runners (monolith, watchtower) and SSH-driven runners (apex):

- k3s cluster provisioning via Terraform
- Service deployment via `kubectl apply`
- Ansible playbook execution — DNS, monitoring, exporters, file shares
- Apex services (Scribe) deployed via SSH + Ansible from GitHub Actions

## Git Workflow

All changes go through **branch → PR → merge**. Direct pushes to `main` are disabled.
...
**Scribe** (`apex:8765`) — Claude's git control plane. Branch, stage, commit, push, and open PRs against this repo — with branch protection and path allowlisting baked in. Deployed as a launchd service on apex.

## Custom Exporters

**T-Mobile Home Internet Exporter** — custom Python Prometheus exporter for T-Mobile FAST 5688W gateway metrics. Hardened systemd service on watchtower.
...
- **Rommie** — local LLM workspace on monolith (pending GPU hardware — RTX 3090 24GB)
